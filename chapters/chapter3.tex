...
Inspite of it's benefits in isolation, most applications use Rust as a drop-in replacement for C. We argue that the true benefits of Rust lie in the fine-grained fault isolation with the use of language-safety features. We use Rust to implement Atmosphere and rely on it's memory safety guarantees for isolation between domains.
